        ...
        try:
            ...
            try:
                ...
                raise KV3DecodeError(
                    "Failed to read KV3 file in text mode (binary magic didn't even match). "
                    + str(text_error)
                ) from text_error
        return rv
...
def write(
    kv3: KV3File | ValueType,
    path_or_stream: str | os.PathLike | typing.IO,
    ...
    *,
    ...
    if use_original_encoding:
        ...
        if encoding is None:
            raise ValueError(
                "Cannot use original encoding if provided kv3 doesn't have one."
            )
    ...
    if encoding == ENCODING_TEXT:
        ...
        if isinstance(fp, io.TextIOBase):
            raise TypeError(
                "Cannot write binary KV3 to a text stream. If this is a file, please open it in binary mode ('wb')."
            )
