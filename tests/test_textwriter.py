...
def test_text_writer_header():
    custom_header = KV3Header(
        Encoding("text2", uuid.UUID(int=0)), Format("generic2", uuid.UUID(int=1))
    )
...
def test_text_writer_writes_dict():
    ...
    class MyKV3Format:
        ...
        c: list = dataclasses.field(
            default_factory=lambda: ["listed_text1", "listed_text2"]
        )

    dataclass_kv3 = KV3File(MyKV3Format())
    dict_kv3 = KV3File(
        {"a": "asd asd", "b": {"inner_b": 3}, "c": ["listed_text1", "listed_text2"]}
    )
...
def util_make_indented_kv3(code_kv3: str, code_indentation_level=1) -> str:
    return (
        default_header
        + "\n"
        + code_kv3.strip()  # undo detached triple quotes
