...
class TestBinaryV1UncompressedWriter(unittest.TestCase):
    ...
    def test_write_match_expected(self):
        kv3_obj = KV3File({"A": 1})
        expect = b'VKV\x03\x00\x05\x86\x1b\xd8\xf7\xc1@\xad\x82u\xa4\x82g\xe7\x14|\x16\x12t\xe9\x06\x98F\xaf\xf2\xe6>\xb5\x907\xe7\x01\x00\x00\x00A\x00\t\x01\x00\x00\x00\x00\x00\x00\x00\x10'
        with io.BytesIO() as file:
    ...
    def test_write_null(self):
        ...
        null_VKV = b'VKV\x03\x00\x05\x86\x1b\xd8\xf7\xc1@\xad\x82u\xa4\x82g\xe7\x14|\x16\x12t\xe9\x06\x98F\xaf\xf2\xe6>\xb5\x907\xe7\x00\x00\x00\x00\x01'
        with io.BytesIO() as file:
            writer = BinaryV1UncompressedWriter(kv3_obj)
            writer.write(file)
            file.seek(0)
