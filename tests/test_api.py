...
def test_api_read_from_path():
    ...
    with pytest.raises(
        keyvalues3.KV3DecodeError, match="Failed to read KV3 file in text mode."
    ):
...
def test_api_read_from_stream():
    ...
    with pytest.raises(
        keyvalues3.KV3DecodeError,
        match="KV3 Text Rule 'kv3' didn't match at 'invalidkv3'",
    ):
...
def test_api_read_binary():
    ...
    # a bad magic
    with pytest.raises(
        keyvalues3.InvalidKV3Magic, match="Invalid binary KV3 magic: b'VDF3'"
    ):
...
def test_api_write():
    ...
    with tempfile.TemporaryFile("w") as fp:
        keyvalues3.write(my_object, getattr(fp, "file", fp))
    ...
    with io.BytesIO(null_VKV) as my_stream:
        keyvalues3.write(None, my_stream, keyvalues3.ENCODING_BINARY_UNCOMPRESSED)
