...
""",
    ...
    def test_multiline_strngs(self):
        with self.assertRaises(Exception):
            KV3TextReader().parse(default_header + '"""')
        with self.assertRaises(Exception):
            KV3TextReader().parse(default_header + '""""')
        with self.assertRaises(Exception):
            KV3TextReader().parse(default_header + '"""""')
        ...
        with self.assertRaises(Exception):
            KV3TextReader().parse(default_header + '"""a\n"""')
        with self.assertRaises(Exception):
            KV3TextReader().parse(default_header + '"""a\n\n"""')
...
""\"
    ...
    def test_prints_back_same_kv3_no_header(self):
        ...
        self.assertEqual(
            self.kv3text,
            kv3textwriter.encode(
                value, kv3textwriter.KV3EncoderOptions(no_header=True)
            ),
        )
...
for kv3file in itertools.chain(docs.glob("*.kv3"), docs.glob("*.vcompmat")):
...
@pytest.mark.parametrize(
    "file,assumed_valid,no_header",
    kv3_files,
    ids=[f"'{f.name}'-{v}{n}" for f, v, n in kv3_files],
)
...
@pytest.mark.skipif(
    resourcecompiler.is_file() == False, reason="resourcecompiler not available"
)
@pytest.mark.parametrize(
    "file,assumed_valid,no_header",
    vpfc_files,
    ids=[f"'{f.name}'-{v}{n}" for f, v, n in vpfc_files],
)
def test_parity(file: Path, assumed_valid: bool, no_header: bool):
    """Check parity with resourcecompiler"""
    result = subprocess.run(
        [resourcecompiler, "-f", file], stdout=subprocess.PIPE, stderr=subprocess.PIPE
    )
    ...
    if not b"a root object of type 'CParticleSystemDefinition'" in result.stdout:
        ...
        if assumed_valid:
            warnings.warn(
                f"File '{file.name}' was assumed valid, but resourcecompiler says it isn't. {result.stdout.decode('utf-8')}",
                UserWarning,
            )
        ...
        if not assumed_valid:
            warnings.warn(
                f"File '{file.name}' was assumed invalid, but resourcecompiler says it's fine.",
                UserWarning,
            )
